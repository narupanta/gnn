...
class HydrogelDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        swell_phi = torch.tensor(sample["swell_time_series"], dtype=torch.float32)  # (T, 1)
        swell_phi_tensor = torch.zeros_like(phi)  # (T, N, 1)
        swell_nodes = node_type[:, 4] == 1        # (N,)
        time = torch.tensor(sample["t"], dtype=torch.float32)
        # Fill swell nodes across all timesteps
        for t in range(len(time)) :
            swell_phi_tensor[t, swell_nodes, :] = swell_phi[t]
