import torch

from torch_geometric.data import Data
from tqdm import tqdm
def rollout(model, data) :
    device = model.device
    # split trajectory into frames
    graphs = []
    for t in range(data.world_pos.shape[0]):
        graph = Data(world_pos=data.world_pos[t], phi = data.phi[t], node_type = data.node_type, target=data.target[t], edge_index=data.edge_index, mesh_pos=data.mesh_pos, time=data.time[t], swelling_phi=data.swelling_phi[t], swelling_phi_rate = data.swelling_phi_rate[t], cells=data.cells, mat_param = data.mat_param)
        graphs.append(graph.to(device))
        # run rollout
    rollout_preds = []
    curr_graph = graphs[0].to(device)
    with torch.no_grad():
        loop = tqdm(range(len(graphs)), desc ="Rollout")
        for t in loop:
            # if t % 10 == 0:
            #     print(f" Rollout step {t}/{len(graphs)}")
            #     # correct curr_graph to ground truth to avoid drift
            #     curr_graph = graphs[t].to(device)
            #     # predict next state
            curr_graph.time = graphs[t].time
            curr_graph.swelling_phi = graphs[t].swelling_phi
            curr_graph.swelling_phi_rate = graphs[t].swelling_phi_rate
            pred_next = model.predict(curr_graph)
            curr_graph.world_pos, curr_graph.phi = pred_next[:, :2], pred_next[:, 2:]
            rollout_preds.append(pred_next)

        rollout_preds = torch.stack(rollout_preds, axis=0) # T, N, 3
        #compute error with respect to ground truth
        gt = data.target.to(device) # T, N, 3
        error = (rollout_preds - gt)**2 # T, N, 3
        rmse_x = torch.sqrt(torch.mean(error[:, :, 0]))
        rmse_y = torch.sqrt(torch.mean(error[:, :, 1]))
        rmse_phi = torch.sqrt(torch.mean(error[:, :, 2]))
        print(f"RMSE Ux: {rmse_x:.6f}, Uy: {rmse_y:.6f}, Phi: {rmse_phi:.6f}")
    return {"time" : data.time,
            "pred" : rollout_preds,
            "gt" : gt,
            "mesh_pos" : data.mesh_pos,
            "cells": data.cells,
            "mat_param": data.mat_param,
            "node_type": data.node_type,
            "swell_phi": data.swelling_phi,
            "swell_phi_rate": data.swelling_phi_rate,
            "rmse_x": rmse_x,
            "rmse_y": rmse_y,
            "rmse_phi": rmse_phi}  

def rollout_history(model, data) :
    device = model.device
    # split trajectory into frames
    graphs = []
    for t in range(data.world_pos.shape[0]):
        graph = Data(world_pos=data.world_pos[t], prev_world_pos=data.prev_world_pos[t], 
                     phi = data.phi[t], prev_phi = data.prev_phi[t], 
                     node_type = data.node_type, target=data.target[t], edge_index=data.edge_index, mesh_pos=data.mesh_pos, time=data.time[t], 
                     swelling_phi=data.swelling_phi[t], 
                     swelling_phi_rate = data.swelling_phi_rate[t], swelling_phi_rate_prev = data.swelling_phi_rate_prev[t], 
                     cells=data.cells, mat_param = data.mat_param)
        graphs.append(graph.to(device))
        # run rollout
    rollout_preds = []
    curr_graph = graphs[0].to(device)
    with torch.no_grad():
        loop = tqdm(range(len(graphs)), desc ="Rollout")
        for t in loop:
            # if t % 1 == 0:
            #     print(f" Rollout step {t}/{len(graphs)}")
                # correct curr_graph to ground truth to avoid drift
                # curr_graph.prev_world_pos = graphs[t].prev_world_pos.to(device)
                # curr_graph.world_pos = graphs[t].world_pos.to(device)
                # curr_graph.prev_phi = graphs[t].prev_phi.to(device)
                # curr_graph.phi = graphs[t].phi.to(device)
                # predict next state
            curr_graph.time = graphs[t].time
            curr_graph.swelling_phi = graphs[t].swelling_phi
            curr_graph.swelling_phi_rate = graphs[t].swelling_phi_rate
            curr_graph.swelling_phi_rate_prev = graphs[t].swelling_phi_rate_prev
            pred_next = model.predict(curr_graph)
            curr_graph.prev_world_pos, curr_graph.prev_phi = curr_graph.world_pos, curr_graph.phi 
            curr_graph.world_pos, curr_graph.phi = pred_next[:, :2], pred_next[:, 2:]
            rollout_preds.append(pred_next)

        rollout_preds = torch.stack(rollout_preds, axis=0) # T, N, 3
        #compute error with respect to ground truth
        gt = data.target.to(device) # T, N, 3
        error = (rollout_preds - gt)**2 # T, N, 3
        rmse_x = torch.sqrt(torch.mean(error[:, :, 0]))
        rmse_y = torch.sqrt(torch.mean(error[:, :, 1]))
        rmse_phi = torch.sqrt(torch.mean(error[:, :, 2]))
        print(f"RMSE Ux: {rmse_x:.6f}, Uy: {rmse_y:.6f}, Phi: {rmse_phi:.6f}")
    return {"time" : data.time,
            "pred" : rollout_preds,
            "gt" : gt,
            "mesh_pos" : data.mesh_pos,
            "cells": data.cells,
            "node_type": data.node_type,
            "swell_phi": data.swelling_phi,
            "swell_phi_rate": data.swelling_phi_rate,
            "rmse_x": rmse_x,
            "rmse_y": rmse_y,
            "rmse_phi": rmse_phi}  

def rollout_temporal(model, data):
    """
    Autoregressive rollout for the TemporalWrapper model.
    Args:
        model: TemporalWrapper instance
        data: Hydrogel trajectory (full sequence)
    Returns:
        dict with predictions, RMSE, etc.
    """
    device = next(model.parameters()).device

    # Build sequence of graphs
    graphs = []
    for t in range(data.world_pos.shape[0]):
        graph = Data(
            world_pos=data.world_pos[t],
            phi=data.phi[t],
            node_type=data.node_type,
            target=data.target[t],
            edge_index=data.edge_index,
            mesh_pos=data.mesh_pos,
            time=data.time[t],
            swelling_phi=data.swelling_phi[t],
            swelling_phi_rate=data.swelling_phi_rate[t],
            cells=data.cells,
            mat_param=data.mat_param
        )
        graphs.append(graph.to(device))

    rollout_preds = []

    # Initialize hidden state as None for first step
    h = None

    # Start from first graph
    curr_graph = graphs[0]

    with torch.no_grad():
        loop = tqdm(range(len(graphs)), desc="Rollout")
        for t in loop:
            # Prepare a sequence from curr timestep onward
            # For autoregressive rollout, we feed only the current state
            graph_seq = [curr_graph]

            # Forward through temporal wrapper
            # Output: [N, T_seq=1, node_out_dim]
            pred_delta_seq = model(graph_seq)  
            pred_delta = pred_delta_seq[:, 0, :]  # remove sequence dimension

            # Update current node features
            curr_graph.world_pos = curr_graph.world_pos + pred_delta[:, :2]
            curr_graph.phi = curr_graph.phi + pred_delta[:, 2:]

            rollout_preds.append(torch.cat([curr_graph.world_pos, curr_graph.phi], dim=-1))

            # Prepare next graph (copy metadata but update positions)
            if t + 1 < len(graphs):
                next_graph = graphs[t + 1]
                # Keep current predicted state for next step
                next_graph.world_pos = curr_graph.world_pos.clone()
                next_graph.phi = curr_graph.phi.clone()
                curr_graph = next_graph

        rollout_preds = torch.stack(rollout_preds, dim=0)  # [T, N, 3]

        # Compute RMSE
        gt = data.target.to(device)  # [T, N, 3]
        error = (rollout_preds - gt) ** 2
        rmse_x = torch.sqrt(torch.mean(error[:, :, 0]))
        rmse_y = torch.sqrt(torch.mean(error[:, :, 1]))
        rmse_phi = torch.sqrt(torch.mean(error[:, :, 2]))
        print(f"RMSE Ux: {rmse_x:.6f}, Uy: {rmse_y:.6f}, Phi: {rmse_phi:.6f}")

    return {
        "time": data.time,
        "pred": rollout_preds,
        "gt": gt,
        "mesh_pos": data.mesh_pos,
        "cells": data.cells,
        "mat_param": data.mat_param,
        "node_type": data.node_type,
        "swell_phi": data.swelling_phi,
        "swell_phi_rate": data.swelling_phi_rate,
        "rmse_x": rmse_x,
        "rmse_y": rmse_y,
        "rmse_phi": rmse_phi
    }
