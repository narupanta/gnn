...
def solve_uniaxial_swell(d_, chi, p, swell_function, bc_config):
    ...
    np.savez(f'/home/fenics/shared/dataset/uniaxial_params_testset/uniaxial_{d_}_{chi}_{p}.npz', 
...
if __name__ == "__main__":
    ...
    d = np.logspace(np.log10(6e-9), np.log10(6e-6), 6)
    chi = [0.18, 0.23, 0.26, 0.33, 0.36, 0.42] #, 0.5]
    period = [10]
    bc_config = {
        'ux': ["left"],
        'uy': ["top", "bottom"],
    ...
    # def swell_function(period, t, sharpness=10, bursts=2, burst_start=60, gap=15):
    #     # Linear ramp contribution (active for 10 <= t <= burst_start)
    #     linear = 0.35 + (0.65 - 0.35) * np.clip((t - 10) / (burst_start - 10), 0, 1)
        
    #     # Max level (target after ramp)
    #     baseline = np.full_like(t, 0.65)
        
    #     # Smooth square wave template (range [-0.15, +0.15])
    #     smooth_square = 0.15 * np.tanh(sharpness * np.cos(2 * np.pi * (t - burst_start) / period))
        
    #     # Shift so the wave oscillates between [0.5, 0.65]
    #     pulse_wave = baseline + (smooth_square - 0.15)  
        
    #     # Initialize gated signal at baseline (flat 0.65)
    #     gated = baseline.copy()
        
    #     # Add bursts
    #     for k in range(bursts):
    #         start = burst_start + k * (period + gap)
    #         end = start + period
    #         mask = (t >= start) & (t < end)
    #         gated[mask] = pulse_wave[mask]
        
    #     # Combine: before 60 s use linear, after 60 s use gated
    #     return linear * (t < burst_start) + gated * (t >= burst_start)
    # swell_function = lambda p, t: swell_function(period=p, t = t, sharpness=5, bursts=2, burst_start=60, gap=30)
    ...
    for d_, chi_, p in product(d, chi, period):
        ...
        with open("uniaxial_log.txt", "a") as f:
            ...
            solve_uniaxial_swell(d_, chi_, p, swell_function, bc_config)
            end_time = time.time()
