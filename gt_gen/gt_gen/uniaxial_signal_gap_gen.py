...
def solve_uniaxial_swell(d_, chi, swell_function, swell_params, bc_config):
    ...
    t = np.concatenate((np.logspace(-5, 1, Nincr + 1), np.linspace(10, 60, Nincr - 150 + 1)[1:], np.linspace(60, 100, Nincr - 150 + 1)[1:]))
    ...
    for (i, dti) in enumerate(np.diff(t)):
        ...
        φ_swell.assign(swell_function(t_current, **swell_params))
    ...
    swell_time_series = np.array([swell_function(ti, **swell_params) for ti in t])
    # t = t_new
    # save the data to a .npz file
    np.savez(f'/home/fenics/shared/dataset/uniaxial_testset/uniaxial_{d_}_{chi}_{swell_function.__name__}_{swell_params}.npz', 
...
if __name__ == "__main__":
    ...
    def step_swell(t, t0=60, hold=10, value=0.35, sharpness=10):
        signal = 0.35 + (0.65 - 0.35) * np.clip((t - 10) / 40, 0, 1)
        if t >= t0 and t < t0 + hold:
            signal = 0.65
        elif t >= t0 + hold:
            t_rel = t - (t0 + hold)
            smooth_factor = 0.5 * (1 + np.tanh(sharpness * (t_rel / 1.0 - 0.5)))
            signal = 0.65 * (1 - smooth_factor) + value * smooth_factor
        return signal

    def ramp_swell(t, t0=60, hold=10, duration=10, start=0.65, end=0.35):
        signal = 0.35 + (0.65 - 0.35) * np.clip((t - 10) / 40, 0, 1)
        if t >= t0 and t < t0 + hold:
            signal = start
        elif t >= t0 + hold:
            signal = np.clip((t - (t0 + hold)) / duration, 0, 1) * (end - start) + start
        return signal

    def staircase_swell(t, t0=60, hold=10, steps=[0.65, 0.55, 0.5, 0.45], step_duration=10):
        signal = 0.35 + (0.65 - 0.35) * np.clip((t - 10) / 40, 0, 1)
        if t >= t0 and t < t0 + hold:
            signal = 0.65
        elif t >= t0 + hold:
            for i, val in enumerate(steps):
                start_time = t0 + hold + i * step_duration
    ...
    def square_wave(period, t, hold=10, sharpness=10):
        signal = 0.35 + (0.65 - 0.35) * np.clip((t - 10) / 40, 0, 1)
        if t >= 60 and t < 60 + hold:
            signal = 0.65
        elif t >= 60 + hold:
            t_rel = t - 60 - hold
            signal = 0.5 + 0.15 * np.tanh(sharpness * np.cos(2 * np.pi * t_rel / period))
        return signal


        
    swell_funcs = [
        (ramp_swell, {"t0":60, "duration":10, "start":0.65, "end":0.45}),
        (ramp_swell, {"t0":60, "duration":15, "start":0.65, "end":0.35}),
        (staircase_swell, {"t0":60, "steps":[0.55,0.35,0.55,0.45], "step_duration":5}),
        (staircase_swell, {"t0":60, "steps":[0.45,0.35,0.45,0.65], "step_duration":5}),
    ]
    #create log file name uniaxial_log.txt if it doesn't exist
    log_file = "uniaxial_log.txt"
    ...
    for d_, chi_, swell_function in product(d, chi, swell_funcs):
        # save runtime in log file if it exists, else create it
        with open("uniaxial_log.txt", "a") as f:
            start_time = time.time()
            solve_uniaxial_swell(d_, chi_, swell_function[0], swell_function[1], bc_config)
            end_time = time.time()
            f.write(f"d: {d_}, chi: {chi_}, period: {swell_function[1]}, runtime: {end_time - start_time} seconds\n") 